...
task_babel () {
  cd client
  watchify -v --debug -t [ babelify --presets [ es2015 ] ] roguelike-relations.js -o public/roguelike-relations.min.js
}
...
task_run_server () {
  docker run --name=rogue-ideas-server --restart=always \
             -p 80:8002 --link=universe-testing:database -d spaxe/rogue-ideas-server
}
